...
class FaqGenerator(BaseGenerator):
    ...
    def _build_faq(self, doc: DocumentModel) -> str:
        ...
        if url:
            ...
            lines.append("")
        lines.append(f"*Auto-generated by [opendocs](https://pypi.org/project/opendocs/) on {now}.*")
        lines.append("")
    ...
    def _infer_faq(self, doc: DocumentModel) -> list[dict[str, str]]:
        ...
        # Q5: Prerequisites?
        if self.kg:
            prereqs = [e.name for e in self.kg.entities if e.entity_type == EntityType.PREREQUISITE]
            if prereqs:
                items.append(
                    {
        ...
        # Q6: License?
        if self.kg:
            licenses = [e.name for e in self.kg.entities if e.entity_type == EntityType.LICENSE_TYPE]
            if licenses:
                items.append(
                    {
