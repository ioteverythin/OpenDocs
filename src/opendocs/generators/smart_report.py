...
def generate_smart_report(
    ...
    try:
        ...
        return GenerationResult(
            format=OutputFormat.WORD,  # reuse enum — just a file result
...
def _build_report(doc: DocumentModel, kg: KnowledgeGraph) -> str:
    ...
    # -- Header --
    lines.append(f"# 🧠 {doc.metadata.repo_name or 'Project'} — Semantic Analysis Report")
    lines.append("")
    lines.append(f"> Generated by **opendocs** on {now}")
    lines.append(f"> Source: `{doc.metadata.repo_url or doc.metadata.source_path or '—'}`")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("## 📊 Knowledge Graph Overview")
    lines.append("")
    ...
    persona_labels = {
        ...
        "investor": "💰 Investor / Business Analysis",
    ...
    for et in EntityType:
        ...
        lines.append("|------|-----------|--------|----------------|")
        for e in entities:
            conf_icon = "🟢" if e.confidence >= 0.8 else ("🟡" if e.confidence >= 0.5 else "🔴")
            lines.append(
    ...
    lines.append("---")
    lines.append(f"*Report generated by opendocs v0.1 • {now}*")
    lines.append("")
