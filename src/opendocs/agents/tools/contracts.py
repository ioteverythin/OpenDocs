    ...
    def validate_params(self, params: dict[str, Any]) -> list[str]:
        ...
        for p in self.parameters:
            ...
            if p.enum and p.name in params and params[p.name] not in p.enum:
                errors.append(f"Parameter '{p.name}' must be one of {p.enum}, got '{params[p.name]}'")
        return errors
    ...
    ToolContract(
        ...
        parameters=[
            ...
            ToolParameter(
                name="chart_type",
                type="string",
                enum=["bar", "line", "pie", "scatter", "heatmap", "treemap"],
            ),
...
FIGMA_CREATE_FRAME = register_tool(
    ToolContract(
        ...
        parameters=[
            ToolParameter(
                name="layout",
                type="object",
                description="Frame layout config (width, height, background)",
            ),
        ...
        output_schema={
            "type": "object",
            "properties": {"frame_id": {"type": "string"}, "figma_url": {"type": "string"}},
        },
        requires_auth=True,
    )
)

FIGMA_ADD_NODES = register_tool(
    ToolContract(
        ...
        parameters=[
            ...
            ToolParameter(
                name="nodes",
                type="array",
                description="List of node specs (type, position, content, style)",
            ),
        ],
        output_type="json",
        output_schema={
            "type": "object",
            "properties": {"node_ids": {"type": "array"}, "figma_url": {"type": "string"}},
        },
        requires_auth=True,
    )
)
    ...
    ToolContract(
        ...
        parameters=[
            ...
            ToolParameter(
                ...
                required=False,
                default="flat-icon",
                enum=["flat-icon", "isometric", "hand-drawn", "technical", "photo-realistic"],
            ),
            ToolParameter(
                name="size",
                type="string",
        ...
        output_schema={
            "type": "object",
            "properties": {"image_url": {"type": "string"}, "local_path": {"type": "string"}},
        },
        requires_auth=True,
    )
)
    ...
    ToolContract(
        ...
        parameters=[
            ToolParameter(
                name="instructions",
                type="string",
                description="What to improve (e.g. 'expand executive summary')",
            ),
            ToolParameter(
                name="references_to_KG",
                type="array",
                description="KG entity/relation IDs to reference",
            ),
            ToolParameter(
                name="source_path",
                type="string",
                description="Path to existing docx",
                required=False,
            ),
        ],
        output_type="docx",
    ...
    ToolContract(
        ...
        parameters=[
            ...
            ToolParameter(
                name="source_path",
                type="string",
                description="Path to existing pptx",
                required=False,
            ),
        ],
        output_type="pptx",
...
CONFLUENCE_PUBLISH = register_tool(
    ToolContract(
        ...
        parameters=[
            ToolParameter(
                name="page_tree_model",
                type="object",
                description="Hierarchical page tree with title, body, children",
            ),
        ...
        output_schema={
            "type": "object",
            "properties": {"page_url": {"type": "string"}, "page_id": {"type": "string"}},
        },
        requires_auth=True,
    )
)
